##################################################################### flowCut: Precise and Accurate Automated Removal of Outlier
##################################################################### Events # and Flagging of Files Based on Time Versus
##################################################################### Fluorescence Analysis # Authors: Justin Meskas and Sherrie
##################################################################### Wang #
flowCut <- function(f, Segment = 500, Channels = NULL, Directory = NULL, 
    ...
    MonotonicFix = NULL, Verbose = FALSE) {
    ...
    if (is.null(FileID)) {
        ...
        # samp <- sample(1:9999, 1) FileID <- paste0(rep(0,
        # 4-nchar(samp)), samp)
        if (Verbose == TRUE) {
    ...
    #### Extracting the location of channels where flowCut will be
    #### applied ####
    ...
    # if (length(Time.loc) >= 2){ Time.loc <- Time.loc[1] #
    # default to take the first.  } names(Time.loc) <- NULL
    ...
    if (length(all.Time.loc) >= 2) {
        ...
        parameters(f)$name[all.Time.loc[which(all.Time.loc != 
            Time.loc)]] <- paste0(parameters(f)$name[all.Time.loc[which(all.Time.loc != 
            Time.loc)]], "-Removed")
        colnames(exprs(f))[all.Time.loc[which(all.Time.loc != 
            Time.loc)]] <- paste0(colnames(exprs(f))[all.Time.loc[which(all.Time.loc != 
            Time.loc)]], "-Removed")
    }
        ...
        if (all(time.data == cummax(time.data)) == FALSE) {
            ...
            if (Plot == "All" || (Plot == "Flagged Only")) {
                ...
                par(mfrow = c(1, 1), oma = c(0, 0, 0, 0), mar = c(5, 
                  5, 3, 1))
                plot(seq_len(length(time.data)), time.data, pch = 19, 
                  cex = 0.2, lty = 2, col = "blue", main = paste0("Monotonic Time Correction: ", 
    ...
    # only for the cases that have channels that are
    # monotonically increasing in themselves. It has nothing to
    # do with monotonically increasing in the time dimension.
    ...
    if (length(MonotonicWithTime) >= 1 && Verbose == TRUE) {
        message(paste0("Channels ", paste0(MonotonicWithTime, 
            collapse = ", "), " are monotonically increasing in time and have been removed from the analysis."))
    }
    ...
    if (length(which(NoVariation == Time.loc)) >= 1) {
        ...
        parameters(f)$name[Time.loc] <- paste0(parameters(f)$name[Time.loc], 
            "-Removed")
        colnames(exprs(f))[Time.loc] <- paste0(colnames(exprs(f))[Time.loc], 
            "-Removed")
        Time.loc <- NULL
        
        # if(length(which(NoVariation != all.Time.loc)) >= 1){ does
        # something exist in all.Time.loc that isn't in NoVariation
    ...
    if (length(Time.loc) == 0) {
        ...
        parameters(f)@data <- rbind(parameters(f)@data, c("Time", 
            "Time", 262144, -111, 262143))
        rownames(parameters(f))[length(colnames(f))] <- paste0("$P", 
            length(colnames(f)))
    ...
    if (range_of_time != 0) {
        ...
        # dividing_points <- seq(min(f@exprs[,Time.loc]),
        # max(f@exprs[,Time.loc]), length.out = 11)
        # uniformity_in_time_test_2 <- sapply (1:10, function(x) {
        # length(intersect( which(f@exprs[,Time.loc] <=
        # dividing_points[x+1]), which(f@exprs[,Time.loc] >=
        # dividing_points[x])) ) })
        
        # This number needs to be set better. Used to be 0.2, moved
        # to 0.22 to work with a particular project.
        ...
        # if ( (min(uniformity_in_time_test_2) <=
        # 0.05*max(uniformity_in_time_test_2)) ){ message('The 2 time
        # channel does not appear to be distributed like an expected
    ...
    # Use all non scatter/time channels unless otherwise
    # specified
    if (!is.null(Channels)) {
    ...
    #### ##########################################
    res.temp <- removeLowDensSections(f, Time.loc = Time.loc, 
    ...
    #### 1st Time: Calculate means and which segments will be
    #### removed #########
    res.temp <- calcMeansAndSegmentsRemoved(f = f, Segment = Segment, 
        ...
        Verbose = Verbose, Time.loc = Time.loc)
    ...
    #### 2nd Time: Calculate means and which segments will be
    #### removed ######### This time we are only interested in
    #### calculations of quantiles, means, mean range percentage and
    #### we do not want to do any deletion
    res.temp <- calcMeansAndSegmentsRemoved(f = f, Segment = Segment, 
    ...
    #### Continuous Check
    #### ##################################################### Check
    #### if there are sudden changes in the mean for the
    #### neighbouring segments. If the difference of the mean of a
    #### given segment to the mean of an adjacent segment divided by
    #### the difference of the 98th percentile and the 2nd
    #### percentile for the whole file is larger than a critical
    #### value, then the file is flagged.
    ...
    #### ###################### If there is a large gradual change
    #### of the means of the fluorescence in all channels, then the
    #### file is flagged.
    ...
    #### Max of Range of Means Divided by Range of Data #### If
    #### there is a very large gradual change of the means of the
    #### fluorescence in at least one channels, then the file is
    #### flagged.
    resTable["Max of % of range of means divided by range of data", 
        ] <- round(max(meanRangePerc2, na.rm = TRUE), digits = 3)
    # use 1 because we want the worst marker before any
    # corrections.
    ...
    # the pngName will be passed through the variable
    # 'AllowFlaggedRerun' for the second run to make the second
    # figure have a suffix.
    ...
    if (Plot == "All" || (Plot == "Flagged Only" && FlaggedOrNot == 
        ...
        if (PrintToConsole == FALSE) {
            if (UseCairo == TRUE) {
                CairoPNG(filename = pngName, width = (z1) * 600, 
                  height = z2 * 600)
            } else {
                png(filename = pngName, width = (z1) * 600, height = z2 * 
                  600)
            }
        ...
        for (x in CleanChan.loc) {
            ...
            lines(x = (timeCentres1), y = storeMeans1[[x]], cex.main = cex.size, 
                cex.lab = cex.size, cex.axis = cex.size, lwd = 4, 
                col = "deeppink2")
        ...
        title(main = paste0(FileID, " ", FlaggedOrNot, " ", PassedMono, 
            PassedCont, PassedMean, PassedMax), outer = TRUE, 
            cex.main = cex.size + 1)
        if (PrintToConsole == FALSE) {
            dev.off()
        } else {
    ...
    if (AllowFlaggedRerun == TRUE && resTable["Has the file passed", 
        ...
        # resTable['Largest continuous jump', ] } if (
        # as.numeric(res_flowCut$data['Mean of % of range of means
        # divided by range of data', ]) < as.numeric(resTable['Mean
        # of % of range of means divided by range of data', ])){
        # resTableOfResTable['Mean of % of range of means divided by
        # range of data', ] <- resTable['Mean of % of range of means
        # divided by range of data', ] } if (
        # as.numeric(res_flowCut$data['Max of % of range of means
        # divided by range of data', ]) < as.numeric(resTable['Max of
        # % of range of means divided by range of data', ])){
        # resTableOfResTable['Max of % of range of means divided by
        # range of data', ] <- resTable['Max of % of range of means
        # divided by range of data', ] }
...
################################################ Remove sections that have a very low density # The sections
################################################ that have a density of less than LowDensityRemoval
################################################ (defaulted at 10%) of the maximum density are removed.  An
################################################ additional 2.5% of the range of these low density regions
################################################ on either side is also removed because it is very common to
################################################ have a burst of events before or after these low density
################################################ sections.  Because the density functions indices do not
################################################ match with the flowFrames indices, density function indices
################################################ need to convert to flowFrame indices.
removeLowDensSections <- function(f, Time.loc, Segment = 500, 
    LowDensityRemoval = 0.1, Verbose = FALSE) {
    ...
    # Time.loc <- which(tolower(f@parameters@data$name) ==
    # 'time') names(Time.loc) <- NULL
    
    # only plays a role if users are using removeLowDensSections
    # independently of flowCut if(length(Time.loc) == 0){
    # message('There is no time channel. removeLowDensSections
    # only works ', 'with a time channel.') return(list(frame=f,
    # rem.ind=NULL)) }
    ...
    # Extracting indices that has density values below 10% or a
    # user-specified threshold
    low.dens.removeIndLowDens <- which(dens.f$y <= LowDensityRemoval * 
        max(dens.f$y))
    
    # Split the consectutive sections into separate elements in
    # the list
    range.low.dens <- split(low.dens.removeIndLowDens, cumsum(c(1, 
        diff(low.dens.removeIndLowDens) != 1)))
    
    # If there are indices left in range.low.dens to be removed
    if (length(range.low.dens) != 0) {
        
        # change groups of indices to ranges
        range.low.dens <- lapply(seq_len(length(range.low.dens)), 
            function(x) {
        ...
        range.low.dens <- lapply(seq_len(length(range.low.dens)), 
            function(x) {
                ...
                # if (range.temp <= 0.25 *(maxTime-minTime)){ # only add
                # buffer if range is less than 25% of the total time range
                # new.range <- c(
                # max(round(range.low.dens[[x]][1]-0.025*range.temp), 1),
                # min(round(range.low.dens[[x]][2]+0.025*range.temp),
                # length(dens.f$y)) ) } else {
                new.range <- c(max(round(range.low.dens[[x]][1]), 
                  1), min(round(range.low.dens[[x]][2]), length(dens.f$y)))
                # }
                return(new.range)
            })
        ...
        # Change time coordinates to flowframe indices,
        # removeIndLowDens contains flowframe indices to be removed
        for (b2 in seq_len(length(range.low.dens))) {
            removeIndLowDens <- c(removeIndLowDens, intersect(which(exprs(f)[, 
                Time.loc] >= range.low.dens[[b2]][1]), which(exprs(f)[, 
                Time.loc] <= range.low.dens[[b2]][2])))
        }
        ...
        if (length(removeIndLowDens) == 0) {
            ...
            removeIndLowDens <- NULL
        } else if ((nrow(f) - length(removeIndLowDens)) < Segment * 
            3) {
            if (Verbose == TRUE) {
            ...
            # Check if the ranges of time coordinates exceed the maxTime
            # and minTime, if so, update the end points
            ...
            if (temp.text[[1]][1] < minTime && temp.text[[1]][2] > 
                minTime) {
            ...
            for (p1 in seq_len(length(temp.text))) {
                temp.text[[p1]] <- paste(round(temp.text[[p1]], 
                  digits = 2), collapse = " to ")
            }
...
###################################################### Calculate means and which segments will be removed # All
###################################################### events are broken down into segments for analysis Eight
###################################################### measures of each segment (mean, median, 5th, 20th, 80th and
###################################################### 95th percentile, second moment(variation) and third moment
###################################################### (skewness)) are calculated. The regions where these eight
###################################################### measures are significantly different from the rest will be
###################################################### removed.
calcMeansAndSegmentsRemoved <- function(f, Segment, CleanChan.loc, 
    FirstOrSecond, MaxValleyHgt, MaxPercCut, MaxContin, MeanOfMeans, 
    MaxOfMeans, GateLineForce, UseOnlyWorstChannels, AmountMeanRangeKeep, 
    AmountMeanSDKeep, RemoveMultiSD, AlwaysClean, Verbose, Time.loc) {
    
    # f is the flow frame FirstOrSecond - parameter used to
    # determine what calculation or process will be performed
    ...
    maxDistJumped <- rep(0, max(CleanChan.loc))
    # Calcuate all means except for the last segment.  Each
    # segment containing 500 (Segments = 500) events
    
    
    for (k in seq_len(totalNumSeg - 1)) {
        temp <- exprs(f)[Segment * (k - 1) + (seq_len(Segment)), 
            Time.loc]
    ...
    k <- totalNumSeg
    temp <- exprs(f)[(Segment * (k - 1) + 1):nrow(exprs(f)), 
        Time.loc]
    timeCentres[k] <- mean(temp)
    
    for (j in CleanChan.loc) {
        segSummary <- matrix(0, totalNumSeg, 8)
        # Calculating the eight features for each segment except the
        # last one and store them in the matrix
        for (k in seq_len(totalNumSeg - 1)) {
            temp <- exprs(f)[Segment * (k - 1) + (seq_len(Segment)), 
                c(j)]
            segSummary[k, ] <- c(quantile(temp, probs = c(5, 
                20, 50, 80, 95)/100), mean(temp), sapply(2:3, 
                function(x) {
                  moment(temp, order = x, center = TRUE)
                }))
        }
        # Calculating the eight features for the last segment and
        # store them in the matrix
        k <- totalNumSeg
        temp <- exprs(f)[(Segment * (k - 1) + 1):nrow(exprs(f)), 
            c(j)]
        segSummary[k, ] <- c(quantile(temp, probs = c(5, 20, 
            50, 80, 95)/100), mean(temp), sapply(2:3, function(x) {
            moment(temp, order = x, center = TRUE)
        }))
        
        storeMeans[[j]] <- segSummary[, 6]
        quantiles[[j]] <- quantile(exprs(f)[, j], probs = c(0, 
            2, 98, 100)/100)
        meanRangePerc[j] <- (max(storeMeans[[j]]) - min(storeMeans[[j]]))/(quantiles[[j]]["98%"] - 
            quantiles[[j]]["2%"])
        
        if (FirstOrSecond == "First") {
            segSummary <- rbind(sapply(seq_len(ncol(segSummary)), 
                function(x) {
                  (segSummary[, x] - mean(segSummary[, x]))/sd(segSummary[, 
                    x])
                }))
            segSummary <- replace(segSummary, is.nan(segSummary), 
                0)
            ...
            cellDeleteExpo <- sapply(seq_len(nrow(cellDeleteExpo)), 
                function(x) {
            ...
            for (l in seq_len(length(storeMeans[[j]]) - 1)) {
                temp.vect[l] <- abs(storeMeans[[j]][l] - storeMeans[[j]][[l + 
                  1]])/(quantiles[[j]]["98%"] - quantiles[[j]]["2%"])
            }
            maxDistJumped[j] <- max(temp.vect)
        }
    }  # End of for-loop
    ...
    if (length(cellDelete) > 0) {
        choosenChans <- NULL
        if (UseOnlyWorstChannels == TRUE) {
            if (length(which(!is.na(meanRangePerc))) >= AmountMeanRangeKeep && 
                AmountMeanRangeKeep >= 1) {
                choosenChans <- sort(unique(c(choosenChans, sapply(sort(meanRangePerc, 
                  decreasing = TRUE)[seq_len(AmountMeanRangeKeep)], 
                  function(x) {
            ...
            if (length(which(!is.na(sdMeans))) >= AmountMeanSDKeep && 
                AmountMeanSDKeep >= 1) {
                choosenChans <- sort(unique(c(choosenChans, sapply(sort(sdMeans, 
                  decreasing = TRUE)[seq_len(AmountMeanSDKeep)], 
                  function(x) {
                    which(x == sdMeans)
                  }))))
            }
        ...
        typeOfGating <- NULL
        # Check if the file is nice, before removing anything.  If it
        # is, then we can avoid removing slivers.
        if ((round(max(maxDistJumped, na.rm = TRUE), digits = 3) < 
            MaxContin) && (round(mean(meanRangePerc, na.rm = TRUE), 
            digits = 3) < MeanOfMeans) && (round(max(meanRangePerc, 
            na.rm = TRUE), digits = 3) < MaxOfMeans) && is.null(GateLineForce) && 
            AlwaysClean == FALSE) {
            densityGateLine <- NULL
            
            range_7SD <- c(mean(cellDelete2) - RemoveMultiSD * 
                sd(cellDelete2), mean(cellDelete2) + RemoveMultiSD * 
                sd(cellDelete2))
            densityGateLine <- range_7SD[2]
            deletedSegments <- union(which(cellDelete2 < range_7SD[1]), 
                which(cellDelete2 > range_7SD[2]))
            typeOfGating <- paste0(typeOfGating, paste0(RemoveMultiSD, 
                "SD"))
            cellDelete2.org <- cellDelete2
        } else {
            # Finding outliers using by plotting density of the 8
            # measures, the index of cells that have 8 measures
            # significantly different than the rest will be returned
            peaks_info <- getPeaks(cellDelete2, tinypeak.removal = 0.1)
            peaks_xind <- peaks_info$Peaks
            peaks_value <- which(density(cellDelete2)$x %in% 
                peaks_xind)
            peaks_value <- density(cellDelete2)$y[peaks_value]
            peak_max <- max(density(cellDelete2)$y)
            
            cellDelete2.org <- cellDelete2
            # Create very similar data that wont have a very large value
            # in the distribution if there was one very obvious outlier.
            # this allows deGate to be able to gate without resolution
            # issues.
            cellDelete2 <- cellDelete2[which(cellDelete2 <= (mean(cellDelete2) + 
                5 * sd(cellDelete2)))]
            
            all_cut <- deGate(cellDelete2, tinypeak.removal = 0.001, 
                all.cuts = TRUE, upper = TRUE, verbose = FALSE, 
                count.lim = 3)
            
            if (!anyNA(all_cut)) {
                # We need to set a limit on peaks because we don't want to
                # overcut
                if (length(peaks_xind) > 1) {
                  ...
                  for (ml in seq_len(length(peaks_xind) - 1)) {
                    ...
                    y_ind <- which(density(cellDelete2)$x %in% 
                      all_cut[ind])
                  ...
                  # Remove the peaks that are too small to be counted as a
                  # separate population
                  if (length(sn) > 0) {
            ...
            all_cut_value <- which(density(cellDelete2)$x %in% 
                all_cut)
            ...
            if (anyNA(peaks_xind)) {
                ...
                if (length(peaks_xind) == 1) {
                  upper_cut <- deGate(cellDelete2, tinypeak.removal = 0.1, 
                    upper = TRUE, use.upper = TRUE, alpha = 0.1, 
                    percentile = 0.95, verbose = FALSE, count.lim = 3)
                  if (!is.na(upper_cut)) {
                    # We don't want the upper cut and the all cut to be the same
                    # because that means we are cutting at the 95 percentile
                    if (length(all_cut) == 1 && upper_cut == 
                      all_cut) {
                      upper_cut <- deGate(cellDelete2, tinypeak.removal = 0.1, 
                        upper = TRUE, use.upper = TRUE, alpha = 0.05, 
                        verbose = FALSE, count.lim = 3)
                      if (upper_cut == all_cut) {
                        upper_cut <- deGate(cellDelete2, tinypeak.removal = 0.1, 
                          upper = TRUE, use.upper = TRUE, alpha = 0.01, 
                          verbose = FALSE, count.lim = 3)
                      }
                    }
                  }
                  
                  if (length(which(all_cut_value < MaxValleyHgt * 
                    peak_max)) == 0) {
                    all_cut_min <- all_cut[1]
                  } else {
                    all_cut_min <- all_cut[min(which(all_cut_value < 
                      MaxValleyHgt * peak_max))]
                  }
                  ...
                  densityGateLine <- max(upper_cut, all_cut_min, 
                    na.rm = TRUE)
                  ...
                  for (t in all_cut) {
                    ...
                    if (length(which(cellDelete2 > t)) <= MaxPercCut * 
                      length(cellDelete2)) {
                      sind <- c(sind, t)
                    }
                  }
    ...
    if (FirstOrSecond == "Second") {
        return(list(deletedSegments = 0, quantiles = quantiles, 
            storeMeans = storeMeans, meanRangePerc = meanRangePerc, 
            timeCentres = timeCentres))
    }
    ...
    if (length(deletedSegments) != 0) {
        ...
        length.range.seg.rem <- sapply(seq_len(length(range.seg.rem)), 
            function(x) {
        ...
        if (length(range.seg.rem.only.5.or.more) >= 1) {
            # Make sure range.seg.keep is first (will be skipped later
            # because it is only length of 1)
            if (min(unlist(range.seg.rem.only.5.or.more)) <= 
                min(unlist(range.seg.keep))) {
                range.seg.keep <- c(0, range.seg.keep)
            }
            # Make sure range.seg.keep is last (will be skipped later
            # because it is only length of 1)
            if (max(unlist(range.seg.rem.only.5.or.more)) >= 
                max(unlist(range.seg.keep))) {
                range.seg.keep <- c(range.seg.keep, max(unlist(range.seg.rem.only.5.or.more)) + 
                  1)
            }
            # Adding buffer region only when the number of consecutive
            # segments in the group to be deleted are quite large
            ...
            for (b2 in seq_len(length(range.seg.rem.only.5.or.more))) {
                ...
                for (j1 in CleanChan.loc) {
                  ...
                  rght.pop <- storeMeans[[j1]][range.seg.keep[[b2 + 
                    1]]]
                  ...
                  for (side.pop in list(left.pop, rght.pop)) {
                    if (length(side.pop) >= 2) {
                      ...
                      # Code below works for good part to be lower than bad part,
                      # so we might need to transform our data
                      if (mean(side.pop) >= mean(main.pop)) {
                        side.pop <- -(side.pop - mean(main.pop)) + 
                          mean(main.pop)
                      }
                      
                      for (k1 in seq_len(length(side.pop))) {
                        if (length(side.pop) <= 2 || sd(side.pop) == 
                          0 || (max(side.pop) - min(side.pop) == 
                          0)) {
                          break
                        }
                        if (tail(side.pop, n = 1) >= (mean(side.pop) + 
                          sd(side.pop))) {
                          if (left.side) {
                            ...
                            adding.Segments <- c(adding.Segments, 
                              range.seg.keep[[b2 + 1]][k1])
                          }
                          # Remove last point because we want to recalculate mean and
                          # sd on progressively cleaner data
                ...
                adding.Segments <- c(adding.Segments, (min(range.seg.rem.only.5.or.more[[b2]]) - 
                  ...
                  1), (max(range.seg.rem.only.5.or.more[[b2]]) + 
                  1):(max(range.seg.rem.only.5.or.more[[b2]]) + 
                  ceiling(length_section/5)))
            }
        }
        if (length(adding.Segments) >= 1) {
            deletedSegments <- sort(unique(c(deletedSegments, 
                adding.Segments[intersect(which(adding.Segments >= 
                  1), which(adding.Segments <= totalNumSeg))])))
        }
    }
    return(list(deletedSegments = deletedSegments, quantiles = quantiles, 
        storeMeans = storeMeans, typeOfGating = typeOfGating, 
        meanRangePerc = meanRangePerc, timeCentres = timeCentres, 
        densityGateLine = densityGateLine, cellDelete2 = cellDelete2.org, 
